...
public class Movie extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int actorID;
		String picture = "", actorList = "", actorName = "";
		...
		try {
			...
			String actors = "SELECT * FROM actors WHERE FIND_IN_SET(" + id + ",moviesIn);";
			ResultSet rs2 = (ResultSet) stmt.executeQuery(actors);
			
			while(rs2.next()){
				actorName = rs2.getString("name");
				picture = rs2.getString("picture");
				actorID = rs2.getInt("id");
				System.out.println(actorID);
				
				actorList = actorList + "" +
						"<div class=\"col-6\">"+
						"	<form id=" + actorID + " action=\"http://107.23.136.184/FirstServlet/Actor\" method='post'>"+
						"	<a href=\"./Actor\" class='list-group-item list-group-item-action' style=\"background-color:white; border-radius: 25px; margin-left: auto; margin-right: auto; width:50%\">"+
						"		<input type='image' src=\"" + picture + "\" class=\"d-block w-100\" alt=\"...\" style='border: 5px solid yellow'>"+
						"		<input type='hidden' name='id' value=" + actorID + ">" + userPass +
						"	</a>"+
						"	</form>"+
						"</div>";	
				}
			...
			while(rs.next()){
				...
				jumbo = jumbo + "" +
						...
						"	  <h1 class=\"display-4\">"+ name + "(" + year + ")" + "</h1>"+
						"		<div class=\"text-center\">"+
						"			<img id= " + id + " src=" + cover + " alt=" + name + " style=\"max-height: 24.75rem;height: 100%; margin-left: auto; margin-right: auto\">"+
						"		</div>"+
						"	  <hr class=\"my-3\">"+
						"	</div>";
			}
